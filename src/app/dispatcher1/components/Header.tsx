...
const Header = () => {
  ...
  useEffect(() => {
    ...
    if (token && roleType === 'terminal1' && name) {
      setUser({ name, role: roleType });
    } else {
      router.push('/'); // Redirect to login if no user data found or role is not terminal1
    }
  }, [router]);
  ...
  return (
    <header className="bg-white p-2 flex justify-between items-center shadow-sm border-gray-200">
      ...
      <div className="">
        <span className="text-gray-900 font-bold text-lg">Welcome back, Gensan Dispatcher</span>
      </div>
